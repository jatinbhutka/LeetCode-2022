...
```py
        # Clarifying Question:
        ### Is it +vs,-ve, can be both?
        ### Its in the range 32-bit int? [-2^31, 2^31 - 1], If outside range return 0

        # rev = 123 or -123

        # Anything outside range we can not fit in memory.
        # Therefore, we need to return before even reaching outside of it.
        ...
        while x:
            if result < MIN_INT/10 or result > MAX_INT/10: # -99%10 = -1, 99%10 = 9
                return 0

            rem = x % 10 if x > 0 else x % -10
            result = result * 10 + rem
            x = int(x/10)           ### THIS IS IMP ####
        return result
```
