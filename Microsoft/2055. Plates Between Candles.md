...
class Solution:
    def platesBetweenCandles(self, s: str, queries: List[List[int]]) -> List[int]:
        ...
        - store closest plate to the left 
        - store closest plate to the right
        """

        prefixPlates = [0]*len(s) # prefix array storing cumulative plates upto given index i 
        candleToLeft = [0]*len(s) # Next candle to Left of given Index
        candleToRight = [0]*len(s) # Next candle to Right of given Index
        ...
        for ind, ch in enumerate(s):
            if ch == '*':
                ...
                leftInd = ind
            prefixPlates[ind] = noOfPlates
            candleToLeft[ind] = leftInd
        ...
        for ind in range(len(s)-1,-1,-1):
            ...
            if ch == '|':
                rightInd = ind
            candleToRight[ind] = rightInd
            
        ans = []
        for start, end in queries:
            leftCandle = candleToRight[start] # closest plate to right of 'start' in s
            rightCandle = candleToLeft[end] # closest plate to left of 'end' in s
            if leftCandle >= rightCandle:
                ans.append(0)
            else:
                ans.append(prefixPlates[rightCandle]-prefixPlates[leftCandle])
        return ans
```
