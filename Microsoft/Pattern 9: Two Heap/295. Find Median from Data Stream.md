...
class MedianFinder:
    def __init__(self):
        self.smallHalf = [] # top Max - maxHeap
        self.largeHalf = [] # top min - minHeap

    def addNum(self, num: int) -> None:
        heappush(self.smallHalf, -1*num)
        
        if self.smallHalf and self.largeHalf and -1*self.smallHalf[0] > self.largeHalf[0]:
            val = -1 * heappop(self.smallHalf)
            heappush(self.largeHalf, val )
        
        if len(self.smallHalf) > len(self.largeHalf) + 1:
            heappush(self.largeHalf, -heappop(self.smallHalf))
        elif len(self.smallHalf) + 1 < len(self.largeHalf):
            heappush(self.smallHalf, -heappop(self.largeHalf))

    def findMedian(self) -> float:
        if len(self.smallHalf) > len(self.largeHalf):
            return -1*self.smallHalf[0]
        elif len(self.smallHalf) < len(self.largeHalf):
            return self.largeHalf[0]
        else:
            return (-1*self.smallHalf[0] + self.largeHalf[0] )/2
