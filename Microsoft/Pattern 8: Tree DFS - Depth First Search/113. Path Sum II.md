...
class Solution:
    def pathSum(self, root: Optional[TreeNode], targetSum: int) -> List[List[int]]:            
        ...
        def helper(node, sumOf, curr):
            if node is None:
                return
            if node.left is None and node.right is None and node.val == sumOf:
                curr.append(node.val)
                result.append(curr.copy())
                curr.pop()
                return
            curr.append(node.val)
            helper(node.left, sumOf-node.val, curr)
            helper(node.right, sumOf-node.val, curr)
            curr.pop()
